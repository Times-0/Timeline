...
class MancalaGame(TableGame):
	...
	def gameOver(self, playerLeft = None):
		if playerLeft is not None and playerLeft in self[:2]:
			...
			Coin(player_id=opponent['id'], transaction=self.Points[OIndex], comment="Coins earned by playing Mancala").save()
			opponent['coins'] += self.Points[OIndex]

		else:
			Coin(player_id=self[0]['id'], transaction=self.Points[0], comment="Coins earned by playing Mancala").save()
			Coin(player_id=self[1]['id'], transaction=self.Points[1], comment="Coins earned by playing Mancala").save()
