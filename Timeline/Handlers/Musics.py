...
def handleSendBroadcast(musicHandler, broadcastTrack):
	...
	for i in range(sharedCount):
		...
		if track.penguin not in broadcastingPenguins:
			...
			broadcastingPenguins.add(track.penguin_id)

	for engine in ENGINES:
		restPenguins = (i for i in engine.roomHandler.getRoomByName('dance') if i['id'] not in broadcastingPenguins)
