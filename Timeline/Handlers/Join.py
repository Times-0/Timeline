...
def handleJoinServer(client, _id, passd, lang):
    ...
    if client.Protocol == AS3_PROTOCOL:
        client.send('lp', client, client['coins'], 0, 1440, int(time() * 1000), client['age'], 0, client['age']-1, member or '', '', client['cache'].playerWidget, client['cache'].mapCategory, client['cache'].igloo)
